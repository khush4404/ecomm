    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-4">
            ...
            <div className="p-4 border-b border-gray-200">
                ...
                <h3 className="font-semibold text-[#1D1D1D] md:text-xl text-base mb-2">{post.title}</h3>
                <p className="text-sm text-text-gray  md:text-base leading-relaxed">{post.content}</p>
            </div>
                ...
                <div className="bg-gray-100">
                    {displayedImages?.map((image, index) => (
                        <div key={index} className="relative overflow-hidden">
                            <img src={image} alt={`Post image ${index + 1}`} className={`w-full object-cover ${imageHeightClass}`} />
                        </div>
                    ))}
                    ...
                    {hasMoreImages && !showAllImages && (
                        <div className="relative">
                            ...
                            >
                                <span className="text-white font-semibold text-lg">+{post.images.length - 1} more</span>
                            </div>
                        </div>
                    )}
            ...
            <div className="px-4 py-3 grid md:grid-cols-3 grid-cols-2  items-center  gap-2">
                <button
                    ...
                    className={`flex items-center justify-center border w-full gap-2 px-4 py-2 rounded-[42px] transition-colors ${isLiked ? "bg-[#F0BA43] text-white" : "hover:bg-text-gray text-[#F0BA43]"
                        }`}
                >
